          ...
          (admin, id) =>
            admin.name.toLowerCase().includes(name.toLowerCase()) && (
              <div key={admin.id} className={style.tableDetail}>
                <h2>{id}</h2>
                <h2>
                  {admin.name.split("").length > 20
                    ? admin.name.slice(0, 20)
                    : admin.name}
                </h2>
                <h2
                  onClick={() => setModalOpenCLick(true)}
                  style={{ cursor: "pointer" }}
                >
                  {!adminConfirm
                    ? notvisiblePassword(admin.password)
                    : admin.password.split("").length > 30
                    ? admin.password.slice(0, 30)
                    : admin.password}
                </h2>
                <div className={style.twoButtons}>
                  {props.admin.adminsPermission === "TRUE" && (
                    <Link
                      to="/admin/delete/admin"
                      onClick={() => props.selectedAnyItem(admin)}
                    >
                      Удалить
                    </Link>
                  )}
