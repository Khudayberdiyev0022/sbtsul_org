...
import { fetchMessagesProducts, selectedAnyItem } from '../../../actions'
...
function Message(props) {
    ...
    const listRender = () => {
        if (name.length > 0) {
            return (
                props.messages
                    ? props.messages.map(message => message.name.toLowerCase().includes(name.toLowerCase()) && (
...
export default connect(mapStateToProps, { fetchMessagesProducts, selectedAnyItem })(Message)
